...
An uncertain programming language would require \textit{source-to-source} transformations at times. For example, one way to evaluate a programs control-flow (\texttt{if-else} conditions) with uncertain inputs is to unravel the control flow, and to evaluate each possible conditional branch uniquely, and then combine the output. For example consider following function
...
An interval evaluation of the function $\text{unc\_flow}([-1,1])$ would be the union of the output of the different branches, the interval union $[-2, -1) \cup [0, 1]$. For a language like Python, this would require an uncertainty pre-compiler (like that proposed in \cite{gray2019computing}) whereby the python code is first transformed using a pre-compiler, and then evaluated in Python. In Julia, this can be handled natively with its metaprogramming capabilities.

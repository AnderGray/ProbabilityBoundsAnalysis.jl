...
function plot_pbox(s ::pbox, fill = true; name = missing, col = missing, heading = missing, plotting = true, save = false, alpha = 0.2, fontsize = 12)
    ...
    if (ismissing(name)); fig = PyPlot.figure(figsize=(10,10)); else fig = PyPlot.figure(name,figsize=(10,10));end
    ...
    PyPlot.xlabel("Distribution range",fontsize = fontsize); PyPlot.ylabel("CDF",fontsize=fontsize);

    if save; PyPlot.savefig("$name.png"); PyPlot.close(fig);end
    PyPlot.ion()
end

plot_pbox(s :: Union{<:Real, Interval{<:Real}}, fill = true; name = missing, col = missing, alpha = 0.2, fontsize = 12) = plot_pbox(makepbox(s), fill, name = name, col = col, alpha = alpha, fontsize = fontsize)
